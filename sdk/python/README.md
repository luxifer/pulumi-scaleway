...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `scaleway` with the `NAME` of your provider.
...
- binary: pulumi-resource-scaleway
  ...
  ignore: []
  ldflags:
  - -X github.com/luxifer/pulumi-scaleway/provider/pkg/version.Version={{.Tag}}
...
├── config.yaml
├── metadata.json
├── pulumi-scaleway_darwin_amd64_v1
│   └── pulumi-resource-scaleway
├── pulumi-scaleway_darwin_arm64
│   └── pulumi-resource-scaleway
├── pulumi-scaleway_linux_amd64_v1
│   └── pulumi-resource-scaleway
├── pulumi-scaleway_linux_arm64
│   └── pulumi-resource-scaleway
├── pulumi-scaleway_windows_amd64_v1
│   └── pulumi-resource-scaleway.exe
└── pulumi-scaleway_windows_arm64
    └── pulumi-resource-scaleway.exe
```
