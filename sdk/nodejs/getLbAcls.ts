 ...
 * const byFrontID = scaleway_lb_frontend_frt01.id.apply(id => scaleway.getLbAcls({
 *     frontendId: id,
 * }));
 * // Find acls by frontend ID and name
 * const byFrontIDAndName = scaleway_lb_frontend_frt01.id.apply(id => scaleway.getLbAcls({
 *     frontendId: id,
...
export function getLbAclsOutput(args: GetLbAclsOutputArgs, opts?: pulumi.InvokeOptions): pulumi.Output<GetLbAclsResult> {
    return pulumi.output(args).apply(a => getLbAcls(a, opts))
}
