 ...
 * const byLBID = scaleway_lb_lb01.id.apply(id => scaleway.getLbBackends({
 *     lbId: id,
 * }));
 * // Find backends by LB ID and name
 * const byLBIDAndName = scaleway_lb_lb01.id.apply(id => scaleway.getLbBackends({
 *     lbId: id,
...
export function getLbBackendsOutput(args: GetLbBackendsOutputArgs, opts?: pulumi.InvokeOptions): pulumi.Output<GetLbBackendsResult> {
    return pulumi.output(args).apply(a => getLbBackends(a, opts))
}
